...
PONG_OUTPUT=$(timeout "$E2E_TIMEOUT" opencode run \
    "Reply with exactly the word PONG and nothing else" 2>&1) || PONG_OUTPUT=""

if echo "$PONG_OUTPUT" | grep -qi "PONG"; then
    pass "opencode run returns PONG"
else
    fail "opencode run returns PONG (output: ${PONG_OUTPUT:0:100})"
fi
...
MATH_OUTPUT=$(timeout "$E2E_TIMEOUT" opencode run \
    ...
    "What is 2+2? Reply with just the number." 2>&1) || MATH_OUTPUT=""

if echo "$MATH_OUTPUT" | grep -q "4"; then
    pass "opencode run --agent pai-researcher returns 4"
else
    fail "opencode run --agent pai-researcher returns 4 (output: ${MATH_OUTPUT:0:100})"
fi
